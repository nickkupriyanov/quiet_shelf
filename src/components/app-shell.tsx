...
export function AppShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-dvh bg-page p-2 text-primary sm:p-4">
      <div className="mx-auto flex min-h-[calc(100dvh-1rem)] w-full max-w-[1680px] flex-col rounded-[20px] border border-border-soft bg-shell shadow-[0_18px_60px_rgba(36,50,65,0.10)] sm:min-h-[calc(100dvh-2rem)]">
        <header className="flex items-center justify-between gap-4 px-4 py-3 sm:px-5">
          ...
          <nav className="hidden items-center gap-1 rounded-full bg-soft p-1 md:flex">
            {navItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="inline-flex min-h-10 items-center gap-2 rounded-full px-4 text-sm font-medium text-secondary transition hover:bg-card hover:text-primary"
              >
        ...
        <main className="flex-1 px-3 pb-24 pt-2 sm:px-5 md:pb-5">{children}</main>
